...
const SideProgressBar = () => {
  ...
  return (
    <aside className="fixed top-1/4 left-0 z-50 flex flex-col gap-4 px-2 py-4 bg-white/90 shadow-md rounded-r-lg backdrop-blur-md">
      {navItems.map(({ id, name, icon }) => (
        <ScrollLink
          ...
          smooth={true}
          duration={500}
          className={`flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-all duration-300 cursor-pointer 
